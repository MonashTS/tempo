...
namespace ttools {
    ...
    /** LCS parameters */
    const vector<double> epsilons = generate_random_real_vector(prng, 15, 0.0, 0.1);

    /** MSM parameters */
    const vector<double> msm_costs = {0, 0.01, 0.5, 1, 10, 100};

    /** TWE parameters */
    const vector<double> twe_nus = generate_random_real_vector(prng, 7, 0.0, maxv);
    const vector<double> twe_lambdas = generate_random_real_vector(prng, 7, 0.0, maxv);

    /** WDTW parameters */
    // Factor for the weights generation
    const vector<double> weight_factors = generate_random_real_vector(prng, 5, 0.0, 1.0);
